# 文档引用关系报告

生成时间: 2026-03-07T15:19:24.908093

## 统计信息

- 总引用数: 74
- 引用源文件数: 25
- 被引用目标文件数: 29

## 被引用最多的文档（Top 10）

- `meta/index/sources.md` (N/A): 13 次引用
- `govs/G01` (N/A): 7 次引用
- `govs/G03` (N/A): 5 次引用
- `meta/agents/agents.md` (N/A): 4 次引用
- `standards/S03` (N/A): 4 次引用
- `govs/knowledge-sources.yaml` (N/A): 4 次引用
- `govs/G04` (N/A): 4 次引用
- `standards/S08` (N/A): 3 次引用
- `standards/S05` (N/A): 3 次引用
- `meta/index/traceability.md` (N/A): 2 次引用

## 孤立文档（未被引用的文档）

- `1-reqs/requirements.md`
- `2-designs/architecture.md`
- `2-designs/architecture/README.md`
- `2-designs/schema/README.md`
- `2-designs/tests/README.md`
- `3-tasks/TK-101260901-基线任务.md`
- `adrs/ADR-10101-基线治理决策.md`
- `adrs/README.md`
- `agents/agents.md`
- `agents/architect-agent.md`
- `agents/developer-agent.md`
- `agents/orchestrator-agent.md`
- `agents/planner-agent.md`
- `agents/release-agent.md`
- `agents/researcher-agent.md`
- `agents/reviewer-agent.md`
- `agents/specifier-agent.md`
- `agents/tester-agent.md`
- `changelogs/changelog-20260302-governance-baseline.md`
- `examples/example-adr-database.md`
- `examples/example-design-auth.md`
- `examples/example-req-auth.md`
- `examples/example-task-auth-001.md`
- `govs/G01-治理与流程.md`
- `govs/G02-项目宪章.md`
- `govs/G03-质量门禁.md`
- `govs/G04-角色职责.md`
- `govs/G05-Agent协作宪章.md`
- `meta/agents/architect-agent.md`
- `meta/agents/developer-agent.md`
- `meta/agents/orchestrator-agent.md`
- `meta/agents/planner-agent.md`
- `meta/agents/release-agent.md`
- `meta/agents/researcher-agent.md`
- `meta/agents/reviewer-agent.md`
- `meta/agents/specifier-agent.md`
- `meta/agents/tester-agent.md`
- `meta/index/reference-report-summary.md`
- `meta/skills/analyze-change-impact-skill.md`
- `meta/skills/analyze-options-skill.md`
- `meta/skills/check-traceability-skill.md`
- `meta/skills/clarify-requirements-skill.md`
- `meta/skills/decompose-tasks-skill.md`
- `meta/skills/draft-adr-skill.md`
- `meta/skills/estimate-effort-skill.md`
- `meta/skills/fetch-knowledge-skill.md`
- `meta/skills/generate-acceptance-test-skill.md`
- `meta/skills/generate-changelog-release-skill.md`
- `meta/skills/generate-code-skill.md`
- `meta/skills/generate-design-skill.md`
- `meta/skills/generate-unit-test-skill.md`
- `meta/skills/maintain-runbook-skill.md`
- `meta/skills/request-decision-skill.md`
- `meta/skills/run-validation-skill.md`
- `meta/skills/security-compliance-review-skill.md`
- `meta/skills/sync-spec-code-skill.md`
- `meta/skills/write-requirements-skill.md`
- `releases/release-20260302-v1.0.0.md`
- `releases/release-plan.md`
- `runbook/runbook-design-rollback.md`
- `runbook/runbook-emergency-release.md`
- `runbook/runbook-requirement-change.md`
- `runbook/runbook.md`
- `skills/analyze-change-impact-skill.md`
- `skills/analyze-options-skill.md`
- `skills/check-traceability-skill.md`
- `skills/clarify-requirements-skill.md`
- `skills/decompose-tasks-skill.md`
- `skills/draft-adr-skill.md`
- `skills/estimate-effort-skill.md`
- `skills/fetch-knowledge-skill.md`
- `skills/generate-acceptance-test-skill.md`
- `skills/generate-changelog-release-skill.md`
- `skills/generate-code-skill.md`
- `skills/generate-design-skill.md`
- `skills/generate-unit-test-skill.md`
- `skills/maintain-runbook-skill.md`
- `skills/request-decision-skill.md`
- `skills/run-validation-skill.md`
- `skills/security-compliance-review-skill.md`
- `skills/skills.md`
- `skills/sync-spec-code-skill.md`
- `skills/write-requirements-skill.md`
- `standards/S01-文档编码规范.md`
- `standards/S02-编码规范.md`
- `standards/S03-文档规范.md`
- `standards/S04-质量保证.md`
- `standards/S05-交付控制.md`
- `standards/S06-证据规范.md`
- `standards/S07-合约数据设计.md`
- `standards/S08-可信合规.md`
- `standards/S09-运营韧性.md`
- `standards/S10-版本号规范.md`
- `templates/adr.template.md`
- `templates/changelog.template.md`
- `templates/design.template.md`
- `templates/gov.template.md`
- `templates/release.template.md`
- `templates/req.template.md`
- `templates/runbook.template.md`
- `templates/standard.template.md`
- `templates/task.template.md`
- `templates/test.template.md`
- `tests/test-baseline-001.md`
- `tests/test-plan.md`